...
class Profile(models.Model, HitCountMixin):
    profile_id = HashidAutoField(
        primary_key=True, salt=f"user_profile{settings.HASHID_FIELD_SALT}", default=None
    )
    ...
    link_to_speaker_image = models.URLField(
        default="",
        blank=True,
        help_text="A link to your professional profile picture to be used when making speaker's announcements",
    )
    profession = models.CharField(
        ...
        default="",
        blank=False,
        help_text="Speaker's profession. eg. Software Developer",
    )
    ...
    biography = MarkdownxField(
        help_text="[Supports Markdown] - Give us a brief biography about yourself",
        default="",
        blank=False,
    )
    city = models.CharField(
        ...
        default="",
        blank=False,
        help_text="City the speaker is from eg. Accra",
    )
    country = CountryField(default="GH", blank=False, blank_label="(select country)")
    contact_number = models.CharField(
        max_length=50,
        help_text="Please include your country code (233).",
        default="",
        blank=True,
        null=True,
    )
    website = models.URLField(
        max_length=200, help_text="Your website/blog URL.", null=True, blank=True
    )
    ...
    linkedin = models.CharField(
        max_length=100,
        null=True,
        help_text="Please your LinkedIn link",
        default="",
        blank=True,
    )
    ...
    def create_unique_slug(self):
        ...
        queryset = Profile.objects.filter(slug__startswith=original_slug).order_by(
            "profile_id"
        )
        slug = original_slug
        counter = 1
        while queryset.filter(slug=slug).exists():
            ...
            if counter > 100:
                raise RuntimeError(
                    f"max slug attempts for {original_slug} exceeded (100)"
                )
        return slug
