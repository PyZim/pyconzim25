...
from health_safety_guideline.mixins import (
    EditOwnHealth_Safety_GuidelineMixin,
    EditOwnLoginMixin,
)
...
def health_safety_guideline(request, year):
    ...
    health_safety_guidelines = Health_Safety_Guideline.objects.filter(
        event_year=event_year
    ).order_by("-date_created")
    # Render the template with Health and Safety Guidelines specific to the given year
    return render(
        request,
        f"{year}/health_safety_guidelines/health_safety_guidelines.html",
        {"health_safety_guidelines": health_safety_guidelines},
    )
...
def health_safety_guideline_edit(request, pk):
    ...
    if request.method == "POST":
        form = Health_Safety_GuidelineForm(
            request.POST, instance=health_safety_guideline
        )
        if form.is_valid():
