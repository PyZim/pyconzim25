...
def teams_view(request, year):
    ...
    for group in ioc_groups:
        ...
        non_leads = group.iocs.filter(event_year=event_year, is_lead=False).order_by(
            "name"
        )
    ...
    ioc_ungrouped_members = IOCMember.objects.filter(
        event_year=event_year, groups__isnull=True
    ).order_by("name")
    ...
    for group in loc_groups:
        leads = group.members.filter(event_year=event_year, is_lead=True).order_by(
            "name"
        )
        non_leads = group.members.filter(event_year=event_year, is_lead=False).order_by(
            "name"
        )
...
def travel_advice(request, year):
    event_year = get_object_or_404(EventYear, year=year)
    travel_advices = Travel_Advice.objects.filter(event_year=event_year).order_by(
        "-date_created"
    )
    ...
    return render(
        request,
        template_name,
        {"travel_advices": travel_advices, "event_year": event_year},
    )
