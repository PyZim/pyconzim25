...
class Event(models.Model):
    name = models.CharField(
        max_length=255, help_text="The name of the event or activity."
    )
    ...
    location = models.CharField(
        max_length=255, help_text="Location of the event.", blank=True, null=True
    )
    event_year = models.ForeignKey(
        EventYear, on_delete=models.CASCADE, help_text="The year this event belongs to."
    )
    is_featured = models.BooleanField(
        default=False, help_text="Indicates if this is a featured event."
    )
