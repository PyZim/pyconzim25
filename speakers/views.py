...
class Speakers(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            queryset.filter(
                ...
                | Q(
                    user__speaking_proposals__status="A",
                    user__speaking_proposals__user_response="A",
                )
            )
            .annotate(
                ...
                sort_priority=Case(
                    ...
                    When(
                        user__proposals__talk_type__in=["Short Talk", "Long Talk"],
                        then=2,
                    ),
    ...
    def get_context_data(self, **kwargs):
        ...
        keynote_speakers = [
            speaker for speaker in speakers if speaker.is_keynote_speaker
        ]
        other_speakers = [
            speaker for speaker in speakers if not speaker.is_keynote_speaker
        ]
        ...
        meta_title = (
            f"Speakers at PyCon Africa {self.kwargs.get('year', 'default')}"
            if keynote_speakers
            else f"Speakers at PyCon Africa {self.kwargs.get('year', 'default')}"
        )
...
class SpeakerDetailView(HitCountDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        talks = Proposal.objects.filter(
            user=self.object.user, status="A", event_year=event_year
        )

        # Collect related speakers without duplication
        related_speakers = (
            Profile.objects.filter(
                user__proposals__status="A", user__proposals__event_year=event_year
            )
            .annotate(
            ...
            .filter(user_accepted=True)
            .exclude(profile_id=self.object.profile_id)
            .distinct()
        )
