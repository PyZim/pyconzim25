...
def submit_talk(request, year):
    ...
    try:
        ...
        submission_periods = CFPSubmissionPeriod.objects.filter(
            ...
            if active_period or profile.is_a_sponsor_or_keynote_speaker:
                ...
                if form.is_valid():
                    ...
                    html_content = render_to_string(
                        "emails/talks/submission_confirmation.html",
                        {"user": request.user, "proposal": proposal},
                    )
                    text_content = strip_tags(html_content)
                    email = EmailMultiAlternatives(
                        subject, text_content, to=[request.user.email]
                    )
...
class TalkList(TemplateView):
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                "submitted_talks": Proposal.objects.filter(
                    user=self.request.user, event_year__year=year
                ),
                "submission_periods": submission_periods,
                "active_period": active_period,
                "is_editable": active_period is not None
                or profile.is_a_sponsor_or_keynote_speaker,
                "is_sponsor_or_keynote": profile.is_a_sponsor_or_keynote_speaker,
            }
        )
...
class TalkView(UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse_lazy(
            "talks:submitted", kwargs={"year": proposal.event_year.year}
        )
...
class TalkDetailView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        uploaded_documents = Document.objects.filter(
            proposal=proposal, document_type="Slide"
        )
        has_uploaded_slide = uploaded_documents.exists()
        latest_slide = (
            uploaded_documents.latest("uploaded_at") if has_uploaded_slide else None
        )
        ...
        if form.is_valid():
            ...
            return redirect(
                reverse(
                    ...
                    kwargs={
                        "year": proposal.event_year.year,
                        "pk": proposal.proposal_id.hashid,
                    },
                )
            )
...
class TalksDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        meta_description = (
            Truncator(proposal.talk_abstract).words(30, truncate="...")
            if proposal.talk_abstract
            else "Join us at PyCon Africa for an insightful talk."
        )
        meta_og_image = (
            speaker_profiles.first().profile_image.url
            if speaker_profiles.exists() and speaker_profiles.first().profile_image
            else "https://res.cloudinary.com/pycon-africa/image/upload/v1722977619/website_storage_location/media/pyconafrica.png"
        )
        ...
        context.update(
            {
                "title": "Talk Details",
                "year": proposal.event_year.year,
                "related_talks": Proposal.objects.filter(
                    status="A", event_year=proposal.event_year
                ).order_by("?")[:5],
...
def recording(request, year=None):
    year = year or timezone.now().year
    event_year = get_object_or_404(EventYear, year=year)
    recordings = Recording.objects.filter(event_year=event_year).order_by(
        "-date_created"
    )
    template_name = f"{year}/talks/recordings.html"
    return render(
        request, template_name, {"recordings": recordings, "event_year": event_year}
    )
...
def proposing(request, year=None):
    year = year or timezone.now().year
    event_year = get_object_or_404(EventYear, year=year)
    proposing_talks = Proposing_talk.objects.filter(event_year=event_year).order_by(
        "-date_created"
    )
    ...
    return render(
        request,
        template_name,
        {"proposing_talks": proposing_talks, "event_year": event_year},
    )
...
def send_speaker_invitation(request, year, pk):
    if request.method == "POST":
        ...
        return render(
            request,
            "2024/talks/speaker_invite_error.html",
...
def accept_invitation(request, year, pk):
    ...
    invitation = get_object_or_404(
        SpeakerInvitation, talk=proposal, invitee=request.user
    )
...
def list_talks_to_review(request, year):
    ...
    try:
        ...
        reviewed_talk_ids = Review.objects.filter(
            reviewer=reviewer, talk__event_year=event_year
        ...
        messages.error(
            request,
            "You don't yet have rights to review proposals, contact the admin to give you the rights",
        )
        context = {"year": year, "no_reviewer_rights": True}
...
def review_talk(request, year, pk):
    ...
    return render(
        request,
        "2024/talks/reviews/talk_review.html",
        {
            "form": form,
            "talk": talk,
            "year": year,
...
def reviewed_talks_by_category(request, year):
    ...
    for category_code, category_label in Proposal.TALK_CATEGORY:
        talks = (
            ...
            .annotate(
                ...
                avg_score=(
                    F("avg_speaker_expertise")
                    + F("avg_depth_of_topic")
                    + F("avg_relevancy")
                    + F("avg_value_or_impact")
                )
                / 4,
                submission_count=Count(
                    "user__proposals", distinct=True
    ...
    return render(
        request,
        "2024/talks/reviews/reviewed_talks_by_category.html",
        {"category_talks_scores": category_talks_scores, "year": year},
    )
...
def reviewed_talks_by_type(request, year):
    ...
    for talk_type_code, talk_type_label in Proposal.TALK_TYPES:
        talks = (
            Proposal.objects.filter(
                event_year=event_year, talk_type=talk_type_code, reviews__isnull=False
            )
            .annotate(
                avg_speaker_expertise=Avg("reviews__sub_scores__speaker_expertise"),
                avg_depth_of_topic=Avg("reviews__sub_scores__depth_of_topic"),
    ...
    return render(
        request,
        "2024/talks/reviews/reviewed_talks_by_type.html",
        {"type_talks_scores": type_talks_scores, "year": year},
    )
...
class TalkReviewDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if reviews.exists():
            ...
            context["is_accepted"] = (
                context["average_score"] >= 4
            )  # Example criteria (I will have to chnage this)
        return context
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user_response == "A":
                ...
                proposal.status = (
                    "A"  # Set status to 'Accepted' if it wasn't set before
                )
