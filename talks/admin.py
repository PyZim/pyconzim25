...
class ProposalResource(resources.ModelResource):
    ...
    user_first_name = fields.Field(
        attribute="user__user_profile__name", column_name="First Name"
    )
    user_last_name = fields.Field(
        attribute="user__user_profile__surname", column_name="Last Name"
    )
    ...
    def dehydrate_multiple_submissions(self, proposal):
        user_talk_count = Proposal.objects.filter(
            user=proposal.user, event_year=proposal.event_year
        ).count()
...
class TalkAdmin(ImportExportModelAdmin):
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                response = HttpResponse(
                    export_data, content_type=selected_format_class().get_content_type()
                )
                response["Content-Disposition"] = (
                    f'attachment; filename="proposals.{selected_format_class().get_extension()}"'
                )
                return response
        else:
    ...
    def export_selected_action(self, request, queryset):
        ...
        return HttpResponseRedirect(
            reverse("admin:export_selected")
            + "?"
            + "&".join([f"id={id}" for id in selected_ids])
        )
...
class ReviewAdmin(admin.ModelAdmin):
    ...
    get_reviewer_name.admin_order_field = (
        "reviewer__user__username"  # Allows column order sorting by username
    )
    ...
    def average_score(self, obj):
        return (
            obj.average_score()
        )  # Assuming 'average_score' is a method on the Review model
    ...
    def rank_in_submissions(self, obj):
        ...
        sorted_proposals = sorted(
            proposals, key=lambda p: p.average_review_score(), reverse=True
        )
        rank = next(
            (i for i, p in enumerate(sorted_proposals, 1) if p == obj.talk), None
        )
        return rank
