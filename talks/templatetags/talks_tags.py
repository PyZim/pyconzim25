...
def user_talks_summary(user):
        ...
        if not submission_period:
            submission_period = next(
                (
                    period
                    for period in submission_periods
                    if period.start_date > timezone.now()
                ),
                None,
            )
    ...
    invited_talks = (
        Proposal.objects.filter(speakers=user).distinct().order_by("-created_date")[:3]
    )

    return {
        "submitted_talks": submitted_talks,
        "invited_talks": invited_talks,
        "active_period": submission_period is not None
        and submission_period.start_date
        <= timezone.now()
        <= submission_period.end_date,
        "upcoming_period": submission_period
        and submission_period.start_date > timezone.now(),
        "event_year": event_year,
    }
...
def invitation_list(context):
    ...
    invitations = SpeakerInvitation.objects.filter(
