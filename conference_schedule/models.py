...
class Day(models.Model):
    conference_day = models.CharField(
        max_length=30,
        unique=True,
        help_text="The name of the conference day (e.g., Day 1, Day 2).",
    )
    actual_date = models.DateField(
        help_text="The actual date of the conference day.",
        default=datetime.date(2024, 9, 9),
    )
...
class Room(models.Model):
    room_name = models.CharField(
        ...
        null=True,
        blank=True,
        help_text="The name or number of the conference room.",
    )
...
class Schedule(models.Model):
    ...
    talk = models.ForeignKey(
        ...
        blank=True,
        null=True,
        help_text="Select a Talk if it's a Speaker giving a talk",
    )
    event = models.CharField(
        max_length=255,
        default="",
        blank=True,
        help_text="The name of the event or activity.",
    )
    event_description = models.TextField(
        blank=True, help_text="A short description of the event."
    )
    is_an_event = models.BooleanField(
        default=False, help_text="Indicate if this is an event instead of a talk."
    )
    fa_icon = models.CharField(max_length=100, default="", blank=True)
    event_url = models.CharField(
        max_length=250, default="", blank=True, help_text="URL to the event"
    )
    external_url = models.CharField(
        max_length=250, default="", blank=True, help_text="External link to the event"
    )

    start_time = models.TimeField(blank=True, null=True)
    end_time = models.TimeField(blank=True, null=True)

    day_session = models.CharField(
        max_length=10, choices=DAY_SESSIONS, default="", blank=True
    )
    allocated_room = models.ForeignKey(
        Room, on_delete=models.CASCADE, blank=True, null=True
    )
    rowspan = models.CharField(
        max_length=10,
        default="",
        blank=True,
        help_text="Use to determine how this talk fits in its time allocation",
    )
    ...
    def __str__(self):
        if self.is_an_event:
            return (
                f"{self.event} in {self.allocated_room}"
        ...
        if self.talk:
            if self.talk.speakers.exists():
                # Safely retrieve either the profile name or fall back to "PyCon "
                speakers_names = ", ".join(
                    [
                        getattr(speaker.user_profile, "name", "PyCon ")
                        for speaker in self.talk.speakers.all()
                    ]
                )
    ...
    def clean(self):
        ...
        if self.talk and self.event:
            raise ValidationError(
                "You cannot have both a talk and an event in the same schedule entry."
            )
...
class ScheduleVisibility(models.Model):
    is_live = models.BooleanField(
        default=False,
        help_text="Indicates if the schedule is live and visible to all users.",
    )
