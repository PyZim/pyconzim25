...
def schedule(request, year):
    ...
    for day in days:
        ...
        schedules[day] = (
            Schedule.objects.filter(conference_day=day)
            .filter(models.Q(talk__event_year=event_year) | models.Q(is_an_event=True))
            .select_related("talk", "talk__user")
        ...
        context.update(
            {
                ...
                "speakers": Profile.objects.filter(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if talk_schedule.talk:
                talk_schedule.talk.event_year = (
                    event_year  # Only set event_year if a talk is selected
                )

            talk_schedule.save()
            return redirect(
                "conference_schedule:schedule", year=year
            )  # Update this with the correct app namespace
    else:
        form = TalkScheduleForm()

    return render(
        request, f"{year}/schedule/create_schedule.html", {"form": form, "year": year}
    )
        ...
        if form.is_valid():
            ...
            if schedule_instance.start_time and schedule_instance.end_time:
                if schedule_instance.start_time >= schedule_instance.end_time:
                    ...
                    return render(
                        request,
                        f"{year}/schedule/create_schedule.html",
                        {"form": form, "year": year},
                    )
