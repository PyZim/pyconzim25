...
class TalkScheduleForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "event_url": forms.URLInput(
                attrs={"class": "form-control", "placeholder": "Enter event URL"}
            ),
            "external_url": forms.URLInput(
                attrs={"class": "form-control", "placeholder": "Enter external URL"}
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        scheduled_talks = Schedule.objects.filter(talk__isnull=False).values_list(
            "talk_id", flat=True
        )
        ...
        if instance and instance.talk:
            current_talk_id = (
                instance.talk.proposal_id
            )  # Assuming 'proposal_id' is the primary key
            # Exclude all scheduled talks except the current one
            available_talks = (
                Proposal.objects.filter(status="A", user_response="A")
                .exclude(pk__in=scheduled_talks)
                .union(Proposal.objects.filter(pk=current_talk_id))
            )
        else:
