...
def show_sponsor(context):
    ...
    return render_to_string(
        template_name,
        {"sponsor": sponsor, "event_year": latest_year},
...
def show_home_sponsor(context):
    sponsor_h_home = Sponsor.objects.filter(
        is_visible=True, tier__name="HEADLINE SPONSOR"
    )
    ...
    latest_year = (
        sponsor_h_home.first().event_year.year if sponsor_h_home.exists() else 2024
    )
