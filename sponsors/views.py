...
def sponsors(request, year):
    ...
    sponsorship_tiers = SponsorshipTier.objects.filter(event_year=event_year).order_by(
        "display_order"
    )
    ...
    for tier in sponsorship_tiers:
        ...
        if tier.sponsors.exists():
            no_sponsors = (
                False  # If any tier has visible sponsors, set no_sponsors to False
            )

    return render(
        request,
        f"{year}/sponsors/sponsors.html",
        {
